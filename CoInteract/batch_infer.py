...
def parse_args():
    ...
    parser.add_argument("--seed_base", type=int, default=0,
                        help="Base seed added to the clip index. Default preserves the old seed=r behavior.")
    parser.add_argument("--face_reference_guidance_scale", type=float, default=0.0,
                        help="Training-free router-masked identity guidance scale")
    parser.add_argument("--face_reference_guidance_power", type=float, default=1.0,
                        help="Power applied to the MoE face/head router mask")
    parser.add_argument("--face_reference_guidance_start_t", type=float, default=0.0,
                        help="Normalized timestep lower bound for face-reference guidance")
    parser.add_argument("--face_reference_guidance_end_t", type=float, default=0.9,
                        help="Normalized timestep upper bound for face-reference guidance")
    parser.add_argument("--face_reference_guidance_counterfactual", type=str, default="zero",
                        choices=["zero", "latent_blur"],
                        help="Weak-ID branch used for router-masked identity delta")
    parser.add_argument("--face_reference_guidance_blur_kernel", type=int, default=5,
                        help="Odd spatial kernel for latent_blur weak-ID counterfactual")
    parser.add_argument("--face_reference_guidance_samg", action="store_true",
                        help="Use Head-SAMG adaptive local scaling for face-reference guidance")
    parser.add_argument("--face_reference_guidance_samg_min_mult", type=float, default=0.5,
                        help="Minimum local multiplier for Head-SAMG relative to face_reference_guidance_scale")
    parser.add_argument("--face_reference_guidance_samg_max_mult", type=float, default=1.5,
                        help="Maximum local multiplier for Head-SAMG relative to face_reference_guidance_scale")
    parser.add_argument("--face_reference_guidance_apg_eta", type=float, default=1.0,
                        help="Velocity-space APG eta for identity delta; 1.0 keeps original delta")
    parser.add_argument("--arcface_guidance_scale", type=float, default=0.0,
                        help="Sparse in-loop ArcFace latent guidance update scale")
    parser.add_argument("--arcface_guidance_steps", type=str, default="",
                        help="Comma-separated denoising step indices where ArcFace guidance runs")
    parser.add_argument("--arcface_guidance_target_cosine", type=float, default=None,
                        help="Stop ArcFace updates at a guidance step once mean cosine reaches this value")
    parser.add_argument("--arcface_guidance_max_updates", type=int, default=1,
                        help="Max ArcFace gradient updates per selected denoising step")
    parser.add_argument("--arcface_guidance_min_updates", type=int, default=0,
                        help="Minimum ArcFace updates before target-cosine early stopping")
    parser.add_argument("--arcface_guidance_decode_lat_frames", type=int, default=1,
                        help="Number of latent frames to decode for each ArcFace guidance update")
    parser.add_argument("--arcface_guidance_score_frame_count", type=int, default=1,
                        help="Number of decoded frames scored by ArcFace per guidance update")
    parser.add_argument("--arcface_guidance_temporal_windows", type=int, default=1,
                        help="Number of temporal windows to score for post ArcFace guidance; <=0 means every latent window")
    parser.add_argument("--arcface_guidance_post_mode", type=str, default="aggregate",
                        choices=["aggregate", "sequential"],
                        help="Post ArcFace mode: aggregate averages windows into one update; sequential optimizes windows one by one")
    parser.add_argument("--arcface_guidance_onnx_path", type=str, default="",
                        help="Path to ArcFace ONNX model for in-loop guidance")
    parser.add_argument("--arcface_guidance_debug_dir", type=str, default="",
                        help="Directory for before/after ArcFace guidance frame sheets")
    parser.add_argument("--arcface_guidance_detector", type=str, default="insightface",
                        choices=["insightface", "fixed"],
                        help="Face crop source for ArcFace guidance/eval debug")
    parser.add_argument("--arcface_guidance_timing", type=str, default="post",
                        choices=["post", "final", "midstep", "both"],
                        help="When to apply ArcFace latent guidance. Default is post-denoising.")
    parser.add_argument("--reference_kv_guidance_scale", type=float, default=0.0,
                        help="Training-free reference-head K/V guidance scale. 0 disables it.")
    parser.add_argument("--reference_kv_guidance_mode", type=str, default="head_attn",
                        choices=["head_attn", "spatial_copy"],
                        help="Reference K/V guidance mode: head attention or lightweight spatial-aligned value copy.")
    parser.add_argument("--reference_kv_guidance_blocks", type=str, default="8:28",
                        help="Block range/list for reference K/V guidance, e.g. '8:28' or '8,12,16'.")
    parser.add_argument("--reference_kv_guidance_start_t", type=float, default=0.05,
                        help="Normalized timestep lower bound for reference K/V guidance.")
    parser.add_argument("--reference_kv_guidance_end_t", type=float, default=0.75,
                        help="Normalized timestep upper bound for reference K/V guidance.")
    parser.add_argument("--reference_kv_guidance_bbox", type=str, default="0.30,0.08,0.70,0.42",
                        help="Normalized head/face box x0,y0,x1,y1 used for reference K/V guidance.")
    ...
    parser.add_argument("--resize_output_to_reference", action="store_true", default=True,
                        help="Resize final MP4 back to the original person reference image size after generation.")
    parser.add_argument("--no_resize_output_to_reference", action="store_false", dest="resize_output_to_reference",
                        help="Keep final MP4 at model canvas size.")
    parser.add_argument("--include_reference_start_frame", action="store_true",
                        help="Keep the pinned reference image frame as the first saved frame instead of dropping it.")
...
def resize_video_to_resolution(video_path: str, target_width: int, target_height: int) -> None:
    ...
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,
        "-vf", f"scale={int(target_width)}:{int(target_height)}",
        "-c:v", "libx264",
        "-pix_fmt", "yuv420p",
        "-c:a", "copy",
        tmp_path,
...
def speech_to_video(pipe, prompts, person_image, audio_path,
                    ...
                    include_reference_start_frame=False,
    ...
    try:
        for r in range(num_repeat):
            ...
            current_clip = pipe(
                ...
                cfg_scale=cfg_scale,
                face_reference_guidance_scale=face_reference_guidance_scale,
                face_reference_guidance_power=face_reference_guidance_power,
                face_reference_guidance_start_t=face_reference_guidance_start_t,
                face_reference_guidance_end_t=face_reference_guidance_end_t,
                face_reference_guidance_counterfactual=face_reference_guidance_counterfactual,
                face_reference_guidance_blur_kernel=face_reference_guidance_blur_kernel,
                face_reference_guidance_samg=face_reference_guidance_samg,
                face_reference_guidance_samg_min_mult=face_reference_guidance_samg_min_mult,
                face_reference_guidance_samg_max_mult=face_reference_guidance_samg_max_mult,
                face_reference_guidance_apg_eta=face_reference_guidance_apg_eta,
                arcface_guidance_scale=arcface_guidance_scale,
                arcface_guidance_steps=arcface_guidance_steps,
                arcface_guidance_target_cosine=arcface_guidance_target_cosine,
                arcface_guidance_max_updates=arcface_guidance_max_updates,
                arcface_guidance_min_updates=arcface_guidance_min_updates,
                arcface_guidance_decode_lat_frames=arcface_guidance_decode_lat_frames,
                arcface_guidance_score_frame_count=arcface_guidance_score_frame_count,
                arcface_guidance_temporal_windows=arcface_guidance_temporal_windows,
                arcface_guidance_post_mode=arcface_guidance_post_mode,
                arcface_guidance_onnx_path=arcface_guidance_onnx_path,
                arcface_guidance_debug_dir=arcface_guidance_debug_dir,
                arcface_guidance_detector=arcface_guidance_detector,
                arcface_guidance_timing=arcface_guidance_timing,
                reference_kv_guidance_scale=reference_kv_guidance_scale,
                reference_kv_guidance_mode=reference_kv_guidance_mode,
                reference_kv_guidance_blocks=reference_kv_guidance_blocks,
                reference_kv_guidance_start_t=reference_kv_guidance_start_t,
                reference_kv_guidance_end_t=reference_kv_guidance_end_t,
                reference_kv_guidance_bbox=reference_kv_guidance_bbox,
            )
            if include_reference_start_frame and r == 0:
    ...
    for idx, row in df.iterrows():
        try:
            ...
            speech_to_video(
                ...
                include_reference_start_frame=args.include_reference_start_frame,
                face_reference_guidance_scale=args.face_reference_guidance_scale,
                face_reference_guidance_power=args.face_reference_guidance_power,
                face_reference_guidance_start_t=args.face_reference_guidance_start_t,
                face_reference_guidance_end_t=args.face_reference_guidance_end_t,
                face_reference_guidance_counterfactual=args.face_reference_guidance_counterfactual,
                face_reference_guidance_blur_kernel=args.face_reference_guidance_blur_kernel,
                face_reference_guidance_samg=args.face_reference_guidance_samg,
                face_reference_guidance_samg_min_mult=args.face_reference_guidance_samg_min_mult,
                face_reference_guidance_samg_max_mult=args.face_reference_guidance_samg_max_mult,
                face_reference_guidance_apg_eta=args.face_reference_guidance_apg_eta,
                arcface_guidance_scale=args.arcface_guidance_scale,
                arcface_guidance_steps=args.arcface_guidance_steps,
                arcface_guidance_target_cosine=args.arcface_guidance_target_cosine,
                arcface_guidance_max_updates=args.arcface_guidance_max_updates,
                arcface_guidance_min_updates=args.arcface_guidance_min_updates,
                arcface_guidance_decode_lat_frames=args.arcface_guidance_decode_lat_frames,
                arcface_guidance_score_frame_count=args.arcface_guidance_score_frame_count,
                arcface_guidance_temporal_windows=args.arcface_guidance_temporal_windows,
                arcface_guidance_post_mode=args.arcface_guidance_post_mode,
                arcface_guidance_onnx_path=args.arcface_guidance_onnx_path,
                arcface_guidance_debug_dir=args.arcface_guidance_debug_dir,
                arcface_guidance_detector=args.arcface_guidance_detector,
                arcface_guidance_timing=args.arcface_guidance_timing,
                reference_kv_guidance_scale=args.reference_kv_guidance_scale,
                reference_kv_guidance_mode=args.reference_kv_guidance_mode,
                reference_kv_guidance_blocks=args.reference_kv_guidance_blocks,
                reference_kv_guidance_start_t=args.reference_kv_guidance_start_t,
                reference_kv_guidance_end_t=args.reference_kv_guidance_end_t,
                reference_kv_guidance_bbox=args.reference_kv_guidance_bbox,
            )
            if args.resize_output_to_reference:
