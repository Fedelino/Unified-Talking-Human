...
OUTPUT_DIR="/data1/workspace/linxinliang/CoInteract/output_videos/finetune_stage1_step500_m2v_eval_nomoe_20260710"
LOG_PATH="/data1/workspace/linxinliang/CoInteract/logs/finetune_stage1_step500_m2v_eval_nomoe_20260710.log"
TMP_DIR="/tmp/finetune_stage1_step500_m2v_eval_nomoe_20260710_rows"
...
for csv_path, row_index in cases:
    ...
    row = rows[row_index]
    sample_id = f"{row['sample_id'].strip()}_stage1_step500_nomoe"
...
echo "===== Stage1 step500 no-MoE M2V eval start $(date '+%F %T') =====" | tee -a "$LOG_PATH"
...
for row_csv in "$TMP_DIR"/*.csv; do
    ...
    python batch_infer_qilin_og_m2v.py \
        ...
        --cfg_scale 7.0 \
        --no_use_moe >>"$LOG_PATH" 2>&1
    echo "[$(date '+%F %T')] completed ${sample_name}" | tee -a "$LOG_PATH"
    sleep 60
done

echo "===== Stage1 step500 no-MoE M2V eval end $(date '+%F %T') =====" | tee -a "$LOG_PATH"
