...
VISIBLE_NPU="1"
OUT_ROOT="$ROOT/output_videos/bachvidlite_case1_reverse_motioncache_20260705"
LOG_ROOT="$ROOT/logs/bachvidlite_case1_reverse_motioncache_20260705"
CACHE_CSV="$ROOT/examples/cointeract_bachvidlite_case1_motion2_target_20260705.csv"
TARGET_CSV="$ROOT/examples/cointeract_bachvidlite_case1_motion1_cache_20260705.csv"
CACHE_OUT="$OUT_ROOT/01_motion2_identity_cache/th_fullbody_001_motion2_bachvidlite_cachem1.mp4"
...
run_case "01_motion2_identity_cache" "$CACHE_CSV"

if [[ ! -s "$CACHE_OUT" ]]; then
  echo "Missing identity cache video: $CACHE_OUT" | tee -a "$LOG_ROOT/02_motion1_with_cache.log"
  exit 1
fi

run_case \
  "02_motion1_with_motion2_cache" \
  "$TARGET_CSV" \
