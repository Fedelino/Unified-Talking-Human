...
LOG_PATH="/data1/workspace/linxinliang/CoInteract/logs/side_by_side_clear2_cointeract_og_cycle_20260625.log"
TMP_DIR="/tmp/side_by_side_clear2_cointeract_og_cycle_20260625_rows"
COOLDOWN_SECONDS=90
MAX_ATTEMPTS=3
...
echo "===== CoInteract clear2 cycle start $(date '+%F %T') =====" | tee -a "$LOG_PATH"
...
for row_csv in "${ROW_CSVS[@]}"; do
    ...
    while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
        ...
        echo "[$(date '+%F %T')] failed ${sample_name} on attempt ${attempt}; cooling down ${COOLDOWN_SECONDS}s" | tee -a "$LOG_PATH"
        sleep "$COOLDOWN_SECONDS"
        attempt=$((attempt + 1))
    done

    if [ "$attempt" -gt "$MAX_ATTEMPTS" ]; then
        echo "[$(date '+%F %T')] giving up on ${sample_name} after ${MAX_ATTEMPTS} attempts" | tee -a "$LOG_PATH"
    fi

    echo "[$(date '+%F %T')] per-case cooldown ${COOLDOWN_SECONDS}s before next launch" | tee -a "$LOG_PATH"
    sleep "$COOLDOWN_SECONDS"
done

echo "===== CoInteract clear2 cycle end $(date '+%F %T') =====" | tee -a "$LOG_PATH"
