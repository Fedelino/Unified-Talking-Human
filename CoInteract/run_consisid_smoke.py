...
def process_batch(args, pipe):
    ...
    pipe.dit.consisid_interval = 2
    pipe.dit.consisid_extractor = LocalFacialExtractor(id_dim=512, vit_dim=1280, depth=4, num_scale=1, num_queries=32, output_dim=2048).to(device=DEVICE, dtype=_dt)
    _n = len(pipe.dit.blocks)//2
    pipe.dit.consisid_cross_attn = _nn.ModuleList([PerceiverCrossAttention(dim=5120, kv_dim=2048).to(device=DEVICE, dtype=_dt) for _ in range(_n)])
    pipe.dit._consisid_enabled = True
    pipe.dit._consisid_id_tokens = torch.randn(1,32,2048, dtype=_dt, device=DEVICE)*0.02   # SMOKE random id_tokens
    print(f"[ConsisID] attached extractor + {_n} cross-attn (random smoke id_tokens)")
