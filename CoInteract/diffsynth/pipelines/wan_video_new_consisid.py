...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        if getattr(self.dit, "_consisid_enabled", False) and not getattr(self, "_consisid_noise_debug_printed", False):
            print(
                "[ConsisID graph/loss-input] "
                f"noise_pred_grad={noise_pred.requires_grad}, "
                f"noise_pred_target_grad={noise_pred_target.requires_grad}, "
                f"training_target_grad={training_target.requires_grad}"
            )
            self._consisid_noise_debug_printed = True
        ...
        if use_face_weighted_loss:
            ...
            diffusion_loss = torch.nn.functional.mse_loss(noise_pred_target.float(), training_target.float())
        diffusion_loss = diffusion_loss * self.scheduler.training_weight(timestep)
        if getattr(self.dit, "_consisid_enabled", False) and not getattr(self, "_consisid_loss_debug_printed", False):
            print(
                "[ConsisID graph/loss-output] "
                f"diffusion_loss_grad={diffusion_loss.requires_grad}, "
                f"diffusion_loss_grad_fn={type(diffusion_loss.grad_fn).__name__ if diffusion_loss.grad_fn is not None else None}"
            )
            self._consisid_loss_debug_printed = True
...
def _consisid_injection_index(dit, block_id):
    """Return the ConsisID cross-attn index for this block, or None."""
    interval = int(getattr(dit, "consisid_interval", 2))
    start = int(getattr(dit, "consisid_start_block", 0))
    if block_id < start or interval <= 0:
        return None
    rel = block_id - start
    if rel % interval != 0:
        return None
    idx = rel // interval
    cross_attn = getattr(dit, "consisid_cross_attn", None)
    if cross_attn is None or idx >= len(cross_attn):
        return None
    return idx
    ...
    **kwargs,
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            # ConsisID generic Wan path: P2V/M2V samples can have no audio_embeds,
            # so they do not dispatch into model_fn_wans2v. Inject the same ID
            # tokens here to keep identity training connected to the loss.
            _idt = getattr(dit, "_consisid_id_tokens", None)
            _ci = _consisid_injection_index(dit, block_id)
            if _idt is not None and _ci is not None:
                _delta = dit.consisid_cross_attn[_ci](_idt, x)
                if not getattr(dit, "_consisid_generic_graph_debug_printed", False):
                    print(
                        "[ConsisID graph/generic] "
                        f"block={block_id}, x_in_grad={x.requires_grad}, "
                        f"id_tokens_grad={_idt.requires_grad}, delta_grad={_delta.requires_grad}, "
                        f"ca_param_grad={next(dit.consisid_cross_attn[_ci].parameters()).requires_grad}"
                    )
                    dit._consisid_generic_graph_debug_printed = True
                x = x + _delta
        if tea_cache is not None:
            tea_cache.store(x)
    ...
    def create_nested_checkpoint_forward(blocks, block_ids, dit, seq_len, audio_g, audio_m, 
        ...
        def forward_fn(x, ctx, tm, freq, _depth_freqs, _depth_ctx):
            ...
            for blk, bid in zip(blocks, block_ids):
                ...
                _ci = _consisid_injection_index(dit, bid)
                if getattr(dit, "_consisid_enabled", False) and getattr(dit, "_consisid_id_tokens", None) is not None and _ci is not None:
                    _vis = x[:, :seq_len]
                    _delta = dit.consisid_cross_attn[_ci](dit._consisid_id_tokens, _vis)
                    if not getattr(dit, "_consisid_s2v_ckpt_graph_debug_printed", False):
                        print(
                            "[ConsisID graph/s2v_ckpt] "
                            f"block={bid}, vis_in_grad={_vis.requires_grad}, "
                            f"id_tokens_grad={dit._consisid_id_tokens.requires_grad}, "
                            f"delta_grad={_delta.requires_grad}, "
                            f"ca_param_grad={next(dit.consisid_cross_attn[_ci].parameters()).requires_grad}"
                        )
                        dit._consisid_s2v_ckpt_graph_debug_printed = True
                    _vis = _vis + _delta
                    x = torch.cat([_vis, x[:, seq_len:]], dim=1)
            return x
        ...
        if use_deepspeed_activation_checkpointing:
            ...
            for blk, bid in zip(current_blocks, current_block_ids):
                ...
                with torch.autograd.graph.save_on_cpu():
                    ...
                    _ci = _consisid_injection_index(dit, bid)
                    if _idt is not None and _ci is not None:
                        _vis = x[:, :seq_len_x_global]
                        _delta = dit.consisid_cross_attn[_ci](_idt, _vis)
                        if not getattr(dit, "_consisid_s2v_ckpt_branch_graph_debug_printed", False):
                            print(
                                "[ConsisID graph/s2v_ckpt_branch] "
                                f"block={bid}, vis_in_grad={_vis.requires_grad}, "
                                f"id_tokens_grad={_idt.requires_grad}, delta_grad={_delta.requires_grad}, "
                                f"ca_param_grad={next(dit.consisid_cross_attn[_ci].parameters()).requires_grad}"
                            )
                            dit._consisid_s2v_ckpt_branch_graph_debug_printed = True
                        _vis = _vis + _delta
                        x = torch.cat([_vis, x[:, seq_len_x_global:]], dim=1)
        elif use_gradient_checkpointing:
            ...
            for blk, bid in zip(current_blocks, current_block_ids):
                ...
                if _idt is not None and _ci is not None:
                    _vis = x[:, :seq_len_x_global]
                    _delta = dit.consisid_cross_attn[_ci](_idt, _vis)
                    if not getattr(dit, "_consisid_s2v_gradckpt_graph_debug_printed", False):
                        print(
                            "[ConsisID graph/s2v_gradckpt] "
                            f"block={bid}, vis_in_grad={_vis.requires_grad}, "
                            f"id_tokens_grad={_idt.requires_grad}, delta_grad={_delta.requires_grad}, "
                            f"ca_param_grad={next(dit.consisid_cross_attn[_ci].parameters()).requires_grad}"
                        )
                        dit._consisid_s2v_gradckpt_graph_debug_printed = True
                    _vis = _vis + _delta
                    x = torch.cat([_vis, x[:, seq_len_x_global:]], dim=1)
        else:
            ...
            for blk, bid in zip(current_blocks, current_block_ids):
                ...
                if _idt is not None and _ci is not None:
                    _vis = x[:, :seq_len_x_global]
                    _delta = dit.consisid_cross_attn[_ci](_idt, _vis)
                    if not getattr(dit, "_consisid_s2v_graph_debug_printed", False):
                        print(
                            "[ConsisID graph/s2v] "
                            f"block={bid}, vis_in_grad={_vis.requires_grad}, "
                            f"id_tokens_grad={_idt.requires_grad}, delta_grad={_delta.requires_grad}, "
                        ...
                        dit._consisid_s2v_graph_debug_printed = True
                    _vis = _vis + _delta
                    x = torch.cat([_vis, x[:, seq_len_x_global:]], dim=1)
