...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        face_loss_weight = float(inputs.pop("face_loss_weight", 1.0) or 1.0)
        ...
        # else:
        # Standard uniform-weight diffusion loss, optionally upweighted on face tokens
        # for the identity-focused second-stage fine-tune.
        use_face_weighted_loss = (
            face_loss_weight > 1.0
            and face_mask is not None
            and bool(face_mask.any().item())
        )
        if use_face_weighted_loss:
            mse_loss_per_pixel = (noise_pred_target.float() - training_target.float()) ** 2
            B, C, T, H, W = noise_pred_target.shape
            patch_h, patch_w = 2, 2
            H_token = H // patch_h
            W_token = W // patch_w
            expected_tokens = T * H_token * W_token
            if face_mask.numel() == expected_tokens:
                face_mask_3d = face_mask.reshape(T, H_token, W_token).float()
                face_mask_spatial = face_mask_3d.repeat_interleave(patch_h, dim=1).repeat_interleave(patch_w, dim=2)
                face_weight_map = 1.0 + (face_loss_weight - 1.0) * face_mask_spatial
                weighted_mse = mse_loss_per_pixel * face_weight_map.unsqueeze(0).unsqueeze(0)
                diffusion_loss = weighted_mse.mean()
            else:
    ...
    def _collect_router_logits_from_dit(self, dit, cfg_merge=False, audio_only=True):
        """
        ...
        # 获取 audio injection block 的索引集合. For face-reference
        # guidance we can request all MoE blocks, because the router itself is
        # the soft spatial head/face prior.
        ...
        for block_idx, block in enumerate(dit.blocks):
            if audio_only and block_idx not in audio_block_idxs:
                continue
    ...
    def __call__(
        ...
        use_audio_native_guidance: Optional[bool] = False,
        ...
        scheduler_global_dwsw: Optional[bool] = False,
        scheduler_global_weight_scheme: Optional[str] = "uniform",
        return_latents: Optional[bool] = False,
        decode_video: Optional[bool] = True,
        # Teacache
        ...
        # Training-free identity guidance: use CoInteract MoE face/head router
        # as a soft mask over an ID-vs-noID velocity difference.
        face_reference_guidance_scale: Optional[float] = 0.0,
        face_reference_guidance_power: Optional[float] = 1.0,
        face_reference_guidance_start_t: Optional[float] = 0.0,
        face_reference_guidance_end_t: Optional[float] = 0.9,
        face_reference_guidance_counterfactual: Optional[str] = "zero",
        face_reference_guidance_blur_kernel: Optional[int] = 5,
        face_reference_guidance_samg: Optional[bool] = False,
        face_reference_guidance_samg_min_mult: Optional[float] = 0.5,
        face_reference_guidance_samg_max_mult: Optional[float] = 1.5,
        face_reference_guidance_apg_eta: Optional[float] = 1.0,
        # Training-free ArcFace latent guidance. This is separate from HJB:
        # it sparsely corrects the current denoising latent and then re-runs
        # CoInteract before the scheduler step.
        arcface_guidance_scale: Optional[float] = 0.0,
        arcface_guidance_steps: Optional[str] = "",
        arcface_guidance_target_cosine: Optional[float] = None,
        arcface_guidance_max_updates: Optional[int] = 1,
        arcface_guidance_min_updates: Optional[int] = 0,
        arcface_guidance_decode_lat_frames: Optional[int] = 1,
        arcface_guidance_score_frame_count: Optional[int] = 1,
        arcface_guidance_temporal_windows: Optional[int] = 1,
        arcface_guidance_post_mode: Optional[str] = "aggregate",
        arcface_guidance_onnx_path: Optional[str] = "",
        arcface_guidance_debug_dir: Optional[str] = "",
        arcface_guidance_detector: Optional[str] = "insightface",
        arcface_guidance_timing: Optional[str] = "post",
        reference_kv_guidance_scale: Optional[float] = 0.0,
        reference_kv_guidance_mode: Optional[str] = "head_attn",
        reference_kv_guidance_blocks: Optional[str] = "8:28",
        reference_kv_guidance_start_t: Optional[float] = 0.05,
        reference_kv_guidance_end_t: Optional[float] = 0.75,
        reference_kv_guidance_bbox: Optional[str] = "0.30,0.08,0.70,0.42",
    ):
        # Scheduler
        ...
        face_reference_guidance_scale = float(face_reference_guidance_scale or 0.0)
        face_reference_guidance_power = max(0.05, float(face_reference_guidance_power or 1.0))
        face_reference_guidance_start_t = float(face_reference_guidance_start_t or 0.0)
        face_reference_guidance_end_t = float(face_reference_guidance_end_t or 0.9)
        face_reference_guidance_counterfactual = str(face_reference_guidance_counterfactual or "zero").strip().lower()
        if face_reference_guidance_counterfactual not in {"zero", "latent_blur"}:
            face_reference_guidance_counterfactual = "zero"
        face_reference_guidance_blur_kernel = max(1, int(face_reference_guidance_blur_kernel or 1))
        if face_reference_guidance_blur_kernel % 2 == 0:
            face_reference_guidance_blur_kernel += 1
        face_reference_guidance_samg = bool(face_reference_guidance_samg)
        face_reference_guidance_samg_min_mult = float(face_reference_guidance_samg_min_mult or 0.5)
        face_reference_guidance_samg_max_mult = float(face_reference_guidance_samg_max_mult or 1.5)
        if face_reference_guidance_samg_min_mult > face_reference_guidance_samg_max_mult:
            face_reference_guidance_samg_min_mult, face_reference_guidance_samg_max_mult = (
                face_reference_guidance_samg_max_mult,
                face_reference_guidance_samg_min_mult,
            )
        face_reference_guidance_apg_eta = float(face_reference_guidance_apg_eta if face_reference_guidance_apg_eta is not None else 1.0)
        use_face_reference_guidance = (
            face_reference_guidance_scale > 0.0
            and not cfg_merge
            and hasattr(self.dit, 'is_moe_enabled')
            and self.dit.is_moe_enabled()
        )
        face_reference_guidance_reported = False
        arcface_guidance_scale = float(arcface_guidance_scale or 0.0)
        arcface_guidance_max_updates = max(1, int(arcface_guidance_max_updates or 1))
        arcface_guidance_min_updates = max(0, int(arcface_guidance_min_updates or 0))
        arcface_guidance_decode_lat_frames = max(1, int(arcface_guidance_decode_lat_frames or 1))
        arcface_guidance_score_frame_count = max(1, int(arcface_guidance_score_frame_count or 1))
        arcface_guidance_temporal_windows = (
            1 if arcface_guidance_temporal_windows is None else int(arcface_guidance_temporal_windows)
        )
        arcface_guidance_post_mode = str(arcface_guidance_post_mode or "aggregate").strip().lower()
        if arcface_guidance_post_mode not in {"aggregate", "sequential"}:
            arcface_guidance_post_mode = "aggregate"
        arcface_guidance_detector = str(arcface_guidance_detector or "insightface").strip().lower()
        arcface_guidance_timing = str(arcface_guidance_timing or "post").strip().lower()
        arcface_guidance_step_ids = set()
        if arcface_guidance_steps:
            for raw_step in str(arcface_guidance_steps).replace(";", ",").split(","):
                raw_step = raw_step.strip()
                if raw_step:
                    arcface_guidance_step_ids.add(int(raw_step))
        use_arcface_guidance = (
            arcface_guidance_scale > 0.0
            and bool(arcface_guidance_onnx_path)
            and person_image is not None
        )
        use_arcface_midstep_guidance = (
            use_arcface_guidance
            and arcface_guidance_timing in {"midstep", "both"}
            and len(arcface_guidance_step_ids) > 0
        )
        use_arcface_post_guidance = (
            use_arcface_guidance
            and arcface_guidance_timing in {"post", "final", "both"}
        )
        arcface_guidance_state = {}
        reference_kv_guidance_scale = float(reference_kv_guidance_scale or 0.0)
        use_reference_kv_guidance = reference_kv_guidance_scale > 0.0
        ...
        inputs_shared = {
            ...
            "input_audio": input_audio, "audio_sample_rate": audio_sample_rate, "s2v_pose_video": s2v_pose_video, "audio_embeds": audio_embeds, "s2v_pose_latents": s2v_pose_latents, "motion_video": motion_video, "motion_latents": motion_latents,
        ...
        inputs_audio_off = None
        if use_audio_native_guidance:
            if "audio_embeds" not in inputs_posi:
                print("[StableAvatar] audio native guidance requested but no audio embeddings were found. Falling back to standard CFG.")
                use_audio_native_guidance = False
            else:
                inputs_audio_off = dict(inputs_posi)
                inputs_audio_off["audio_embeds"] = 0.0 * inputs_posi["audio_embeds"]
                print(f"[StableAvatar] audio native guidance enabled (text_cfg={cfg_scale}, audio_cfg={cfg_scale_audio})")
        ...
        for dit_model in (self.dit, self.dit2):
            if dit_model is None or not hasattr(dit_model, "set_reference_kv_guidance"):
                continue
            if use_reference_kv_guidance:
                dit_model.set_reference_kv_guidance(
                    scale=reference_kv_guidance_scale,
                    mode=reference_kv_guidance_mode,
                    blocks=reference_kv_guidance_blocks,
                    start_t=reference_kv_guidance_start_t,
                    end_t=reference_kv_guidance_end_t,
                    bbox=reference_kv_guidance_bbox,
                )
            else:
                dit_model.clear_reference_kv_guidance()
        ...
        def _split_model_output(model_output):
            depth_pred_local = None
            if isinstance(model_output, tuple):
                if len(model_output) == 2:
                    model_output, depth_pred_local = model_output
                elif len(model_output) == 3:
                    model_output, depth_pred_local, _ = model_output
            return model_output, depth_pred_local

        def _face_reference_guidance_strength(timestep_tensor):
            if not use_face_reference_guidance:
                return 0.0
            t_norm = float((timestep_tensor.flatten()[0] / self.scheduler.num_train_timesteps).detach().float().cpu().item())
            if t_norm < face_reference_guidance_start_t or t_norm > face_reference_guidance_end_t:
                return 0.0
            return face_reference_guidance_scale

        def _make_noid_shared_inputs(base_shared):
            noid_shared = dict(base_shared)
            noid_latents = base_shared["latents"].clone()

            def _gaussian_blur_latent(ref_like):
                if face_reference_guidance_blur_kernel <= 1:
                    return ref_like
                b, c, t, h, w = ref_like.shape
                flat = ref_like.permute(0, 2, 1, 3, 4).reshape(b * t, c, h, w)
                kernel = face_reference_guidance_blur_kernel
                sigma = max(float(kernel) / 3.0, 1e-3)
                coords = torch.arange(kernel, device=flat.device, dtype=torch.float32) - (kernel - 1) / 2.0
                weights_1d = torch.exp(-(coords ** 2) / (2.0 * sigma * sigma))
                weights_1d = weights_1d / weights_1d.sum()
                weight_h = weights_1d.view(1, 1, kernel, 1).repeat(c, 1, 1, 1).to(dtype=flat.dtype)
                weight_w = weights_1d.view(1, 1, 1, kernel).repeat(c, 1, 1, 1).to(dtype=flat.dtype)
                flat = F.conv2d(flat, weight_h, padding=(kernel // 2, 0), groups=c)
                flat = F.conv2d(flat, weight_w, padding=(0, kernel // 2), groups=c)
                return flat.reshape(b, t, c, h, w).permute(0, 2, 1, 3, 4)

            if face_reference_guidance_counterfactual == "latent_blur":
                ref_latents = noid_latents[:, :, 0:1]
                ref_latents = _gaussian_blur_latent(ref_latents)
                noid_latents[:, :, 0:1] = ref_latents
            else:
                noid_latents[:, :, 0:1] = 0
            noid_shared["latents"] = noid_latents
            if "first_frame_latents" in noid_shared:
                if face_reference_guidance_counterfactual == "latent_blur":
                    weak_first = noid_shared["first_frame_latents"]
                    weak_first = _gaussian_blur_latent(weak_first)
                    noid_shared["first_frame_latents"] = weak_first
                else:
                    noid_shared["first_frame_latents"] = torch.zeros_like(noid_shared["first_frame_latents"])
            return noid_shared

        def _face_reference_apply_samg(delta_id, face_mask):
            if not face_reference_guidance_samg:
                return face_mask * delta_id
            energy = delta_id.float().square().mean(dim=1, keepdim=True)
            active = face_mask.float() > 0.05
            if not bool(active.any().detach().cpu().item()):
                return face_mask * delta_id
            active_energy = energy[active]
            if active_energy.numel() < 2:
                return face_mask * delta_id
            active_cpu = active_energy.detach().float().cpu()
            e_lo = torch.quantile(active_cpu, 0.05).to(device=energy.device, dtype=energy.dtype)
            e_hi = torch.quantile(active_cpu, 0.95).to(device=energy.device, dtype=energy.dtype)
            e_norm = ((energy - e_lo) / (e_hi - e_lo + 1e-8)).clamp(0.0, 1.0)
            local_mult = (
                face_reference_guidance_samg_max_mult
                - e_norm * (face_reference_guidance_samg_max_mult - face_reference_guidance_samg_min_mult)
            )
            return face_mask * local_mult.to(dtype=delta_id.dtype) * delta_id

        def _face_reference_apply_apg(delta_id, base_pred):
            # Velocity-space APG approximation. eta=1.0 is exactly the original delta.
            if abs(face_reference_guidance_apg_eta - 1.0) < 1e-6:
                return delta_id
            delta_f = delta_id.float()
            base_f = base_pred.float()
            dims = tuple(range(1, delta_f.ndim))
            coeff = (
                (delta_f * base_f).sum(dim=dims, keepdim=True)
                / (base_f.square().sum(dim=dims, keepdim=True) + 1e-8)
            )
            delta_parallel = coeff * base_f
            delta_orth = delta_f - delta_parallel
            delta_apg = delta_orth + face_reference_guidance_apg_eta * delta_parallel
            return delta_apg.to(dtype=delta_id.dtype)

        def _arcface_bbox_pixels():
            # First-pass full-body face box for our selected reference framing.
            return (
                int(width * 0.36),
                int(height * 0.14),
                int(width * 0.57),
        ...
        def _arcface_even_frame_indices(num_frames, sample_count):
            if num_frames <= 1 or sample_count <= 1:
                return [max(0, num_frames // 2)]
            count = min(int(sample_count), int(num_frames))
            return sorted({round(i * (num_frames - 1) / (count - 1)) for i in range(count)})

        def _arcface_latent_mask_like(latents, bbox):
            mask = torch.zeros_like(latents[:, :1])
            _, _, T, H_lat, W_lat = mask.shape
            x1, y1, x2, y2 = bbox
            lx1 = max(0, min(W_lat - 1, int(round(x1 / max(width, 1) * W_lat))))
            lx2 = max(lx1 + 1, min(W_lat, int(round(x2 / max(width, 1) * W_lat))))
            ly1 = max(0, min(H_lat - 1, int(round(y1 / max(height, 1) * H_lat))))
            ly2 = max(ly1 + 1, min(H_lat, int(round(y2 / max(height, 1) * H_lat))))
            mask[:, :, 1:, ly1:ly2, lx1:lx2] = 1.0
            return mask

        def _arcface_pil_from_tensor(frame_rgb_m1p1):
            arr = ((frame_rgb_m1p1.detach().float().clamp(-1, 1) + 1.0) * 127.5)
            arr = arr.permute(1, 2, 0).byte().cpu().numpy()
            return Image.fromarray(arr)

        def _arcface_save_debug(debug_dir, progress_id, update_id, before_frames, after_frames, frame_idxs, cos_before, cos_after):
            if not debug_dir:
                return
            os.makedirs(debug_dir, exist_ok=True)
            panels = []
            for idx in frame_idxs:
                before = _arcface_pil_from_tensor(before_frames[idx]).resize((240, 416))
                after = _arcface_pil_from_tensor(after_frames[idx]).resize((240, 416))
                canvas = Image.new("RGB", (480, 416), (20, 20, 20))
                canvas.paste(before, (0, 0))
                canvas.paste(after, (240, 0))
                draw = ImageDraw.Draw(canvas)
                draw.rectangle([0, 0, 479, 32], fill=(0, 0, 0))
                draw.text(
                    (6, 8),
                    f"step {progress_id} upd {update_id} frame {idx} before {cos_before:.3f} after {cos_after:.3f}",
                    fill=(255, 255, 255),
                )
                panels.append(canvas)
            if not panels:
                return
            sheet = Image.new("RGB", (480, 416 * len(panels)), (20, 20, 20))
            for i, panel in enumerate(panels):
                sheet.paste(panel, (0, 416 * i))
            sheet.save(os.path.join(debug_dir, f"arcface_step{progress_id:03d}_upd{update_id:02d}.jpg"), quality=92)

        def _arcface_append_score(debug_dir, progress_id, update_id, frame_idxs, cos_before, cos_min_before,
                                  cos_after=None, cos_min_after=None, updated=False, note=""):
            if not debug_dir:
                return
            os.makedirs(debug_dir, exist_ok=True)
            path = os.path.join(debug_dir, "guidance_scores.csv")
            write_header = not os.path.exists(path)
            with open(path, "a", encoding="utf-8") as f:
                if write_header:
                    f.write(
                        "progress_id,update_id,decoded_frame_idxs,cos_before_mean,cos_before_min,"
                        "cos_after_mean,cos_after_min,updated,note\n"
                    )
                frame_text = "|".join(str(i) for i in frame_idxs)
                after_mean = "" if cos_after is None else f"{cos_after:.6f}"
                after_min = "" if cos_min_after is None else f"{cos_min_after:.6f}"
                safe_note = str(note).replace(",", ";")
                f.write(
                    f"{progress_id},{update_id},{frame_text},{cos_before:.6f},{cos_min_before:.6f},"
                    f"{after_mean},{after_min},{int(bool(updated))},{safe_note}\n"
                )

        def _arcface_get_state():
            if arcface_guidance_state:
                return arcface_guidance_state
            from hjb_face_opt import DifferentiableArcFace, _crop_faces
            arc = DifferentiableArcFace(arcface_guidance_onnx_path, device=self.device, dtype=torch.float32)
            bbox = _arcface_bbox_pixels()
            detector = None
            if arcface_guidance_detector == "insightface":
                try:
                    ...
                    print(
                        f"[ArcFaceGuidance] InsightFace unavailable ({exc}); falling back to fixed bbox",
                        flush=True,
                    )
                    detector = None
            ref_image = person_image.convert("RGB")
            ref_np = np.array(ref_image)
            ref_bbox = _arcface_detect_bbox_np(ref_np, detector) if detector is not None else bbox
            ref_tensor = torch.from_numpy(ref_np.astype("float32")).permute(2, 0, 1).unsqueeze(0)
            ref_tensor = ref_tensor.to(device=self.device, dtype=torch.float32) / 127.5 - 1.0
            ref_crop = _crop_faces(ref_tensor, ref_bbox)
            ref_emb = arc.embed(ref_crop).detach()
            arcface_guidance_state.update({
                "arc": arc,
                "crop_faces": _crop_faces,
                "bbox": bbox,
                "ref_bbox": ref_bbox,
                "ref_emb": ref_emb,
                "detector": detector,
            ...
            print(
                "[ArcFaceGuidance] enabled: "
                f"scale={arcface_guidance_scale}, timing={arcface_guidance_timing}, "
                f"detector={arcface_guidance_detector}, steps={sorted(arcface_guidance_step_ids)}, "
                f"target={arcface_guidance_target_cosine}, max_updates={arcface_guidance_max_updates}, "
                f"decode_lat_frames={arcface_guidance_decode_lat_frames}, "
                f"temporal_windows={arcface_guidance_temporal_windows}, "
                f"post_mode={arcface_guidance_post_mode}, "
                f"debug_dir={arcface_guidance_debug_dir}",
                flush=True,
            )
            return arcface_guidance_state

        def _arcface_tensor_to_rgb_np(frame_rgb_m1p1):
            arr = ((frame_rgb_m1p1.detach().float().clamp(-1, 1) + 1.0) * 127.5)
            return arr.permute(1, 2, 0).byte().cpu().numpy()

        def _arcface_detect_bbox_np(frame_rgb_uint8, detector):
            if detector is None:
                return _arcface_bbox_pixels()
            try:
                frame_bgr = frame_rgb_uint8[:, :, ::-1].copy()
                faces = detector.get(frame_bgr)
            except Exception as exc:
                print(f"[ArcFaceGuidance] detector failed ({exc}); using fixed bbox", flush=True)
                return _arcface_bbox_pixels()
            if not faces:
                return _arcface_bbox_pixels()
        ...
        def _arcface_detect_bboxes_for_frames(frames_rgb_m1p1, frame_idxs, detector):
            bboxes, valid_frame_idxs, valid_local_idxs = [], [], []
            for local_idx, frame_idx in enumerate(frame_idxs):
                if frame_idx < 0 or frame_idx >= frames_rgb_m1p1.shape[0]:
                    continue
                frame_np = _arcface_tensor_to_rgb_np(frames_rgb_m1p1[frame_idx])
                bbox = _arcface_detect_bbox_np(frame_np, detector)
                if bbox is None:
                    continue
                bboxes.append(bbox)
                valid_frame_idxs.append(frame_idx)
                valid_local_idxs.append(local_idx)
            return bboxes, valid_frame_idxs, valid_local_idxs

        def _arcface_crop_frames(frames_rgb_m1p1, bboxes, crop_faces):
            crops = []
            for i, bbox in enumerate(bboxes):
                crops.append(crop_faces(frames_rgb_m1p1[i:i + 1], bbox))
            return torch.cat(crops, dim=0)

        def _arcface_union_bbox(bboxes):
        ...
        def _arcface_decode_subset(latents_for_decode, start_idx=0, decode_lat_frames=None):
            total_len = int(latents_for_decode.shape[2])
            start_idx = max(0, min(int(start_idx), max(total_len - 1, 0)))
            decode_len = min(int(decode_lat_frames or arcface_guidance_decode_lat_frames), total_len - start_idx)
            self.load_models_to_device(["vae"])
            return self.vae.decode(latents_for_decode[:, :, start_idx:start_idx + decode_len], device=self.device, tiled=True)

        def _arcface_temporal_window_starts(latents_for_decode):
            total_len = int(latents_for_decode.shape[2])
            decode_len = min(arcface_guidance_decode_lat_frames, total_len)
            if total_len <= decode_len:
                return [0]
            max_start = total_len - decode_len
            if arcface_guidance_temporal_windows <= 0:
                return list(range(max_start + 1))
            if arcface_guidance_temporal_windows <= 1:
                return [0]
            starts = [
                int(round(max_start * i / max(arcface_guidance_temporal_windows - 1, 1)))
                for i in range(arcface_guidance_temporal_windows)
            ]
            return sorted(set(max(0, min(max_start, s)) for s in starts))

        def _arcface_clear_cache():
            gc.collect()
            if self.device == "npu":
                torch.npu.empty_cache()
        ...
        def _arcface_score_post_windows(candidate_latents, window_starts, ref, crop_faces, detector, arc,
                                        debug_window_start=None):
            cos_sum = 0.0
            cos_count = 0
            cos_min = None
            debug_fr = None
            with torch.no_grad():
                for start_idx in window_starts:
                    frames = _arcface_decode_subset(candidate_latents, start_idx=start_idx)
                    fr = frames[0].permute(1, 0, 2, 3)
                    frame_idxs = _arcface_even_frame_indices(fr.shape[0], arcface_guidance_score_frame_count)
                    bboxes, frame_idxs, _ = _arcface_detect_bboxes_for_frames(fr, frame_idxs, detector)
                    if debug_window_start is not None and start_idx == debug_window_start:
                        debug_fr = fr.detach()
                    if bboxes:
                        faces = _arcface_crop_frames(fr[frame_idxs], bboxes, crop_faces)
                        emb = arc.embed(faces.float())
                        ref_score = ref.to(device=emb.device, dtype=emb.dtype)
                        cos_tensor = (emb @ ref_score.T).squeeze(-1)
                        cos_sum += float(cos_tensor.sum().detach().cpu().item())
                        cos_count += int(cos_tensor.numel())
                        this_min = float(cos_tensor.min().detach().cpu().item())
                        cos_min = this_min if cos_min is None else min(cos_min, this_min)
                        del faces, emb, ref_score, cos_tensor
                    del frames, fr
            if cos_count == 0:
                return None, None, debug_fr
            return cos_sum / cos_count, cos_min, debug_fr

        def _arcface_guidance_update(latents, velocity, progress_id):
            if not use_arcface_midstep_guidance or progress_id not in arcface_guidance_step_ids:
                return latents, False
            state = _arcface_get_state()
            arc = state["arc"]
            crop_faces = state["crop_faces"]
            bbox = state["bbox"]
            ref_emb = state["ref_emb"]
            detector = state.get("detector")
            sigma = self.scheduler.sigmas[progress_id].to(device=latents.device, dtype=latents.dtype)
            updated = latents.detach()
            was_updated = False
            for update_id in range(arcface_guidance_max_updates):
                with torch.enable_grad():
                    z = updated.detach().clone().requires_grad_(True)
                    clean = z - sigma * velocity.detach()
                    frames_before = _arcface_decode_subset(clean)
                    fr_before = frames_before[0].permute(1, 0, 2, 3)
                    frame_idxs = _arcface_even_frame_indices(fr_before.shape[0], arcface_guidance_score_frame_count)
                    bboxes, frame_idxs, _ = _arcface_detect_bboxes_for_frames(fr_before, frame_idxs, detector)
                    if not bboxes:
                        _arcface_append_score(
                            arcface_guidance_debug_dir,
                            progress_id,
                            update_id,
                            [],
                            float("nan"),
                        ...
                        break
                    faces = _arcface_crop_frames(fr_before[frame_idxs], bboxes, crop_faces)
                    emb = arc.embed(faces.float())
                    ref = F.normalize(ref_emb.to(device=emb.device, dtype=emb.dtype), dim=-1)
                    cos = (emb @ ref.T).squeeze(-1)
                    loss = (1.0 - cos).mean()
                    cos_before = float(cos.mean().detach().cpu().item())
                    cos_min = float(cos.min().detach().cpu().item())
                    if (
                        arcface_guidance_target_cosine is not None
                        and update_id >= arcface_guidance_min_updates
                        and cos_before >= float(arcface_guidance_target_cosine)
                    ):
                        _arcface_append_score(
                            arcface_guidance_debug_dir,
                            progress_id,
                            ...
                            cos_min,
                            updated=False,
                            note="target_reached_before_update",
                        )
                        print(
                            f"[ArcFaceGuidance] step={progress_id} update={update_id} "
                            f"target reached before update frame_idxs={frame_idxs} "
                            f"cos_mean={cos_before:.6f} cos_min={cos_min:.6f}",
                            flush=True,
                        )
                        break
                    grad = torch.autograd.grad(loss, z)[0]
                    update_bbox = _arcface_union_bbox(bboxes)
                    mask = _arcface_latent_mask_like(z, update_bbox).to(device=grad.device, dtype=grad.dtype)
                    rms = torch.sqrt(torch.mean((grad * mask).float() ** 2) + 1e-8).to(dtype=grad.dtype)
                    step = arcface_guidance_scale * mask * grad / rms
                    candidates = []
                    for step_mult in (1.0, 0.25, 0.0625, 0.015625):
                        scaled_step = step * step_mult
                        candidates.append((f"minus_grad_x{step_mult:g}", (z - scaled_step).detach().to(dtype=latents.dtype)))
                        candidates.append((f"plus_grad_x{step_mult:g}", (z + scaled_step).detach().to(dtype=latents.dtype)))
                    best_name = "rejected"
                    best_updated = z.detach().to(dtype=latents.dtype)
                    best_fr_after = fr_before.detach()
                    best_cos = cos_before
                    best_cos_min = cos_min
                    for candidate_name, candidate_latents in candidates:
                        clean_after = candidate_latents - sigma * velocity.detach()
                        frames_after = _arcface_decode_subset(clean_after)
                        fr_after = frames_after[0].permute(1, 0, 2, 3)
                        faces_after = _arcface_crop_frames(fr_after[frame_idxs], bboxes, crop_faces)
                        emb_after = arc.embed(faces_after.float())
                        cos_after_tensor = (emb_after @ ref.T).squeeze(-1)
                        candidate_cos = float(cos_after_tensor.mean().detach().cpu().item())
                        candidate_cos_min = float(cos_after_tensor.min().detach().cpu().item())
                        if candidate_cos > best_cos:
                            best_name = candidate_name
                            best_updated = candidate_latents
                            best_fr_after = fr_after.detach()
                            best_cos = candidate_cos
                            best_cos_min = candidate_cos_min
                        del clean_after, frames_after, fr_after, faces_after, emb_after
                    updated = best_updated
                    fr_after = best_fr_after
                    cos_after = best_cos
                    cos_after_min = best_cos_min
                    update_accepted = best_name != "rejected"
                    _arcface_save_debug(
                        arcface_guidance_debug_dir,
                        progress_id,
                        update_id,
                    ...
                    _arcface_append_score(
                        ...
                        cos_min,
                        cos_after=cos_after,
                        cos_min_after=cos_after_min,
                        updated=update_accepted,
                        note=best_name,
                    )
                    print(
                        f"[ArcFaceGuidance] step={progress_id} update={update_id + 1}/{arcface_guidance_max_updates} "
                        f"frame_idxs={frame_idxs} cos_mean {cos_before:.6f}->{cos_after:.6f} "
                        f"cos_min {cos_min:.6f}->{cos_after_min:.6f} direction={best_name}",
                        flush=True,
                    )
                    was_updated = was_updated or update_accepted
                    del z, clean, frames_before, fr_before, faces, emb, loss, grad, mask
                    del fr_after
                    gc.collect()
                    if self.device == "npu":
                        torch.npu.empty_cache()
                    ...
                    if (
                        arcface_guidance_target_cosine is not None
                        and update_id + 1 >= arcface_guidance_min_updates
                        and cos_after >= float(arcface_guidance_target_cosine)
                    ):
                        print(
                            f"[ArcFaceGuidance] step={progress_id} target reached after update "
                            f"cos_mean={cos_after:.6f}",
                            flush=True,
                        )
                        break
            return updated, was_updated

        def _arcface_post_guidance_update(latents):
            if not use_arcface_post_guidance:
                return latents, False
            state = _arcface_get_state()
            arc = state["arc"]
            crop_faces = state["crop_faces"]
            ...
            updated = latents.detach()
            was_updated = False
            if arcface_guidance_post_mode == "sequential":
                for update_id in range(arcface_guidance_max_updates):
                    window_starts = _arcface_temporal_window_starts(updated)
                    accepted_count = 0
                    scored_count = 0
                    pass_before_sum = 0.0
                    pass_after_sum = 0.0
                    pass_before_min = None
                    pass_after_min = None
                    for window_i, start_idx in enumerate(window_starts):
                        debug_update_id = update_id * 1000 + window_i
                        with torch.enable_grad():
                            z = updated.detach().clone().requires_grad_(True)
                            ref = F.normalize(ref_emb.detach().reshape(1, -1), dim=-1)
                            frames_before = _arcface_decode_subset(z, start_idx=start_idx)
                            fr_before = frames_before[0].permute(1, 0, 2, 3)
                            frame_idxs = _arcface_even_frame_indices(fr_before.shape[0], arcface_guidance_score_frame_count)
                            bboxes, frame_idxs, _ = _arcface_detect_bboxes_for_frames(fr_before, frame_idxs, detector)
                            score_frame_ids = [f"lat{start_idx}:f{idx}" for idx in frame_idxs]
                            if not bboxes:
                                _arcface_append_score(
                                    arcface_guidance_debug_dir,
                                    -1,
                                    ...
                                    float("nan"),
                                    updated=False,
                                    note=f"seq_lat{start_idx}_no_face_detected",
                                )
                                del z, frames_before, fr_before
                                _arcface_clear_cache()
                                continue
                            faces = _arcface_crop_frames(fr_before[frame_idxs], bboxes, crop_faces)
                            emb = arc.embed(faces.float())
                            ref_loss = ref.to(device=emb.device, dtype=emb.dtype)
                            cos = (emb @ ref_loss.T).squeeze(-1)
                            loss = (1.0 - cos).mean()
                            cos_before = float(cos.mean().detach().cpu().item())
                            cos_min = float(cos.min().detach().cpu().item())
                            scored_count += 1
                            pass_before_sum += cos_before
                            pass_before_min = cos_min if pass_before_min is None else min(pass_before_min, cos_min)
                            if (
                                arcface_guidance_target_cosine is not None
                                and update_id >= arcface_guidance_min_updates
                                and cos_before >= float(arcface_guidance_target_cosine)
                                ...
                                _arcface_append_score(
                                    ...
                                    cos_min,
                                    updated=False,
                                    note=f"seq_lat{start_idx}_target_reached_before_update",
                                )
                                pass_after_sum += cos_before
                                pass_after_min = cos_min if pass_after_min is None else min(pass_after_min, cos_min)
                                del z, frames_before, fr_before, faces, emb, ref_loss, cos, loss
                                _arcface_clear_cache()
                                continue
                            grad = torch.autograd.grad(loss, z)[0]
                            update_bbox = _arcface_union_bbox(bboxes)
                            ...
                            for step_mult in (1.0, 0.25, 0.0625, 0.015625):
                                scaled_step = step * step_mult
                                candidates.append((f"post_seq_minus_grad_x{step_mult:g}", (z - scaled_step).detach().to(dtype=latents.dtype)))
                                candidates.append((f"post_seq_plus_grad_x{step_mult:g}", (z + scaled_step).detach().to(dtype=latents.dtype)))
                            best_name = "post_seq_rejected"
                            best_updated = z.detach().to(dtype=latents.dtype)
                            best_fr_after = fr_before.detach()
                            best_cos = cos_before
                            best_cos_min = cos_min
                            for candidate_name, candidate_latents in candidates:
                                candidate_cos, candidate_cos_min, candidate_fr_after = _arcface_score_post_windows(
                                    candidate_latents,
                                    [start_idx],
                                    ref,
                                    crop_faces,
                                ...
                                if candidate_cos > best_cos:
                                    best_name = candidate_name
                                    best_updated = candidate_latents
                                    best_fr_after = candidate_fr_after.detach() if candidate_fr_after is not None else best_fr_after
                                    best_cos = candidate_cos
                                    best_cos_min = candidate_cos_min
                                _arcface_clear_cache()
                            updated = best_updated
                            cos_after = best_cos
                            cos_after_min = best_cos_min
                            update_accepted = best_name != "post_seq_rejected"
                            accepted_count += int(update_accepted)
                            was_updated = was_updated or update_accepted
                            pass_after_sum += cos_after
                            pass_after_min = cos_after_min if pass_after_min is None else min(pass_after_min, cos_after_min)
                            _arcface_save_debug(
                                arcface_guidance_debug_dir,
                                -1,
                                debug_update_id,
                                fr_before.detach(),
                                best_fr_after.detach(),
                                frame_idxs,
                                cos_before,
                                cos_after,
                            )
                            _arcface_append_score(
                                ...
                                cos_min_after=cos_after_min,
                                updated=update_accepted,
                                note=f"seq_lat{start_idx}_{best_name}",
                            )
                            print(
                                f"[ArcFaceGuidance] post sequential pass={update_id + 1}/{arcface_guidance_max_updates} "
                                f"window={window_i + 1}/{len(window_starts)} lat_start={start_idx} "
                                f"cos_mean {cos_before:.6f}->{cos_after:.6f} "
                                f"cos_min {cos_min:.6f}->{cos_after_min:.6f} direction={best_name}",
                                flush=True,
                            )
                            del z, frames_before, fr_before, faces, emb, ref_loss, cos, loss
                            del grad, mask, step, candidates, best_updated, best_fr_after
                            _arcface_clear_cache()
                    if scored_count > 0:
                        pass_before_mean = pass_before_sum / scored_count
                        pass_after_mean = pass_after_sum / scored_count
                        print(
                            f"[ArcFaceGuidance] post sequential pass={update_id + 1} summary "
                            f"windows={len(window_starts)} scored={scored_count} accepted={accepted_count} "
                            f"cos_mean {pass_before_mean:.6f}->{pass_after_mean:.6f} "
                            f"cos_min {pass_before_min:.6f}->{pass_after_min:.6f}",
                            flush=True,
                        )
                    if accepted_count == 0:
                        print(
                            f"[ArcFaceGuidance] post sequential pass={update_id + 1} accepted no updates; stopping",
                            flush=True,
                        )
                        break
                return updated, was_updated
            for update_id in range(arcface_guidance_max_updates):
                with torch.enable_grad():
                    z = updated.detach().clone().requires_grad_(True)
                    window_starts = _arcface_temporal_window_starts(z)
                    ref = F.normalize(ref_emb.detach().reshape(1, -1), dim=-1)
                    grad_accum = None
                    valid_windows = 0
                    cos_sum = 0.0
                    ...
                    for start_idx in window_starts:
                        ...
                        cos = (emb @ ref_loss.T).squeeze(-1)
                        loss = (1.0 - cos).mean()
                        grad_piece = torch.autograd.grad(loss, z)[0]
                        grad_accum = grad_piece.detach() if grad_accum is None else grad_accum + grad_piece.detach()
                        valid_windows += 1
                        cos_sum += float(cos.sum().detach().cpu().item())
                        cos_count += int(cos.numel())
                        this_min = float(cos.min().detach().cpu().item())
                        cos_min = this_min if cos_min is None else min(cos_min, this_min)
                        all_bboxes.extend(bboxes)
                        score_frame_ids.extend(f"lat{start_idx}:f{idx}" for idx in frame_idxs)
                        if debug_fr_before is None:
                            debug_fr_before = fr_before.detach()
                            debug_frame_idxs = list(frame_idxs)
                            debug_window_start = start_idx
                        del frames_before, fr_before, faces, emb, ref_loss, cos, loss, grad_piece
                        _arcface_clear_cache()

                    if grad_accum is None or cos_count == 0 or not all_bboxes:
                        _arcface_append_score(
                            arcface_guidance_debug_dir,
                            -1,
                            update_id,
                            ...
                            float("nan"),
                            updated=False,
                            note="post_no_face_detected",
                        )
                        break
                    grad = grad_accum / max(valid_windows, 1)
                    cos_before = cos_sum / cos_count
                    if (
                        arcface_guidance_target_cosine is not None
                        and update_id >= arcface_guidance_min_updates
                        and cos_before >= float(arcface_guidance_target_cosine)
                        ...
                        _arcface_append_score(
                            ...
                            cos_min,
                            updated=False,
                            note="post_target_reached_before_update",
                        )
                        print(
                            f"[ArcFaceGuidance] post update={update_id} target reached before update "
                            f"windows={window_starts} cos_mean={cos_before:.6f} cos_min={cos_min:.6f}",
                            flush=True,
                        )
                        break
                    update_bbox = _arcface_union_bbox(all_bboxes)
                    mask = _arcface_latent_mask_like(z, update_bbox).to(device=grad.device, dtype=grad.dtype)
                    rms = torch.sqrt(torch.mean((grad * mask).float() ** 2) + 1e-8).to(dtype=grad.dtype)
                    step = arcface_guidance_scale * mask * grad / rms
                    candidates = []
                    for step_mult in (1.0, 0.25, 0.0625, 0.015625):
                        scaled_step = step * step_mult
                        candidates.append((f"post_minus_grad_x{step_mult:g}", (z - scaled_step).detach().to(dtype=latents.dtype)))
                        candidates.append((f"post_plus_grad_x{step_mult:g}", (z + scaled_step).detach().to(dtype=latents.dtype)))
                    best_name = "post_rejected"
                    best_updated = z.detach().to(dtype=latents.dtype)
                    best_fr_after = debug_fr_before.detach() if debug_fr_before is not None else None
                    best_cos = cos_before
                    best_cos_min = cos_min
                    for candidate_name, candidate_latents in candidates:
                        candidate_cos, candidate_cos_min, candidate_fr_after = _arcface_score_post_windows(
                    ...
                    cos_after = best_cos
                    cos_after_min = best_cos_min
                    update_accepted = best_name != "post_rejected"
                    if debug_fr_before is not None and fr_after is not None and debug_frame_idxs:
                        _arcface_save_debug(
                            arcface_guidance_debug_dir,
                            -1,
                            update_id,
                            debug_fr_before.detach(),
                            fr_after.detach(),
                            debug_frame_idxs,
                            cos_before,
                            cos_after,
                    ...
                    print(
                        f"[ArcFaceGuidance] post update={update_id + 1}/{arcface_guidance_max_updates} "
                        f"windows={window_starts} cos_mean {cos_before:.6f}->{cos_after:.6f} "
                        f"cos_min {cos_min:.6f}->{cos_after_min:.6f} direction={best_name}",
                        flush=True,
                    )
                    was_updated = was_updated or update_accepted
                    del z, grad, grad_accum, mask, step, candidates, best_updated
                    del debug_fr_before, fr_after
                    _arcface_clear_cache()
                    if (
                        arcface_guidance_target_cosine is not None
                        and update_id + 1 >= arcface_guidance_min_updates
                        ...
                        print(
                            f"[ArcFaceGuidance] post target reached after update cos_mean={cos_after:.6f}",
                            flush=True,
                        )
                        break
            return updated, was_updated

        def _router_face_mask_to_latents(router_weights_by_block, pred_tensor):
            """Convert MoE router face weights [base, hand, face] to latent mask.

            Router tokens are patch-grid tokens for target frames first, followed
            by reference/product tokens. The denoising prediction includes a
            clean-reference placeholder at frame 0, so the returned mask prepends
            a zero reference slice and only guides target frames.
            """
            if not router_weights_by_block:
                return None
            if pred_tensor.ndim != 5 or pred_tensor.shape[2] <= 1:
                return None

            batch, _, total_frames, latent_h, latent_w = pred_tensor.shape
            target_frames = total_frames - 1
            patch_h = max(1, latent_h // 2)
            patch_w = max(1, latent_w // 2)
            target_tokens = target_frames * patch_h * patch_w

            face_weights = []
            for weights in router_weights_by_block.values():
                if weights is None or weights.ndim != 3 or weights.shape[-1] < 3:
                    continue
                if weights.shape[1] < target_tokens:
                    continue
                face = weights[:batch, :target_tokens, 2].to(device=pred_tensor.device, dtype=torch.float32)
                face_weights.append(face)
            if not face_weights:
                return None

            face = torch.stack(face_weights, dim=0).mean(dim=0).clamp(0.0, 1.0)
            face = face.reshape(batch, target_frames, patch_h, patch_w).unsqueeze(1)
            if patch_h != latent_h or patch_w != latent_w:
                import torch.nn.functional as F
                face = F.interpolate(face, size=(target_frames, latent_h, latent_w), mode="trilinear", align_corners=False)
            if abs(face_reference_guidance_power - 1.0) > 1e-6:
                face = face.clamp(0.0, 1.0).pow(face_reference_guidance_power)
            ref_zeros = torch.zeros((batch, 1, 1, latent_h, latent_w), device=pred_tensor.device, dtype=face.dtype)
            face = torch.cat([ref_zeros, face], dim=2)
            return face.to(dtype=pred_tensor.dtype)

        def _slice_audio_embeds_for_window(audio_embeds_value, start, length, raw_per_latent=4):
            if audio_embeds_value is None or not torch.is_tensor(audio_embeds_value):
                return audio_embeds_value
            raw_start = int(start) * int(raw_per_latent)
            raw_len = int(length) * int(raw_per_latent) + 1
            raw_end = raw_start + raw_len
            sliced = audio_embeds_value[..., raw_start:raw_end]
            current_len = sliced.shape[-1]
            if current_len < raw_len:
                pad_len = raw_len - current_len
                pad_source = sliced[..., -1:] if current_len > 0 else audio_embeds_value[..., -1:]
                sliced = torch.cat([sliced, pad_source.repeat(*([1] * (pad_source.ndim - 1)), pad_len)], dim=-1)
            return sliced

        def _slice_target_condition_for_window(tensor_value, start, end):
            if tensor_value is None or not torch.is_tensor(tensor_value):
                return tensor_value
            length = int(end) - int(start)
            sliced = tensor_value[:, :, start:end]
            if sliced.shape[2] < length:
                pad_len = length - sliced.shape[2]
                pad_source = sliced[:, :, -1:] if sliced.shape[2] > 0 else torch.zeros_like(tensor_value[:, :, :1])
                sliced = torch.cat([sliced, pad_source.repeat(1, 1, pad_len, 1, 1)], dim=2)
            return sliced

        def _slice_cfg_inputs_for_window(cfg_inputs, start, length):
            if cfg_inputs is None:
                return None
            window_inputs = dict(cfg_inputs)
            if "audio_embeds" in window_inputs:
                window_inputs["audio_embeds"] = _slice_audio_embeds_for_window(window_inputs["audio_embeds"], start, length)
            return window_inputs

        def _scheduler_dwsw_weights(overlap_len, device, dtype):
            if overlap_len <= 0:
                return None
            if overlap_len == 1:
                weights = torch.ones((1,), device=device, dtype=torch.float32)
            elif scheduler_global_weight_scheme == "log":
                weights = torch.linspace(0.0, 1.0, overlap_len, device=device, dtype=torch.float32)
                weights = torch.log1p(weights * (torch.exp(torch.ones((), device=device)) - 1.0))
                weights = (weights - weights.min()) / torch.clamp(weights.max() - weights.min(), min=1e-6)
            else:
                weights = torch.linspace(0.0, 1.0, overlap_len, device=device, dtype=torch.float32)
            return weights.to(dtype=dtype).view(1, 1, overlap_len, 1, 1)

        def _make_scheduler_window_inputs(base_shared, latents_all, start, end):
            target_len = int(end) - int(start)
            window_shared = dict(base_shared)
            ref_latents = latents_all[:, :, 0:1]
            target_latents = latents_all[:, :, 1 + start:1 + end]
            window_shared["latents"] = torch.cat([ref_latents, target_latents], dim=2)
            window_shared["sliding_window_size"] = None
            window_shared["sliding_window_stride"] = None
            if window_shared.get("s2v_pose_latents") is not None:
                window_shared["s2v_pose_latents"] = _slice_target_condition_for_window(
                    window_shared["s2v_pose_latents"], start, end
                )
            if window_shared.get("depth_latents") is not None:
                window_shared["depth_latents"] = _slice_target_condition_for_window(
                    window_shared["depth_latents"], start, end
                )
            # Keep motion memory as-is. In the current true-global TIA path it is
            # zero/drop-motion, while the target timeline itself is windowed.
            if window_shared["latents"].shape[2] != target_len + 1:
                raise RuntimeError("Scheduler DWSW window slicing produced an invalid latent length.")
            return window_shared

        def _run_model_cfg(window_shared, timestep_tensor, start):
            target_len = window_shared["latents"].shape[2] - 1
            noise_pred_posi = self.model_fn(
                **models,
                **window_shared,
                **_slice_cfg_inputs_for_window(inputs_posi, start, target_len),
                timestep=timestep_tensor,
                is_conditional=True,
                depth_mutual_visible=depth_mutual_visible,
            )
            noise_pred_posi, depth_pred_posi = _split_model_output(noise_pred_posi)

            if cfg_scale != 1.0 or (use_audio_native_guidance and cfg_scale_audio != 1.0):
                if cfg_merge:
                    raise NotImplementedError(
                        "scheduler_global_dwsw does not support cfg_merge=True. "
                        "Use cfg_merge=False so each window can be stepped independently."
                    )
                noise_pred_nega = self.model_fn(
                    **models,
                    **window_shared,
                    **_slice_cfg_inputs_for_window(inputs_nega, start, target_len),
                    timestep=timestep_tensor,
                    is_conditional=True,
                    depth_mutual_visible=depth_mutual_visible,
                )
                noise_pred_nega, _ = _split_model_output(noise_pred_nega)

                if use_audio_native_guidance and inputs_audio_off is not None:
                    noise_pred_audio_off = self.model_fn(
                        **models,
                        **window_shared,
                        **_slice_cfg_inputs_for_window(inputs_audio_off, start, target_len),
                        timestep=timestep_tensor,
                        is_conditional=True,
                        depth_mutual_visible=depth_mutual_visible,
                    )
                    noise_pred_audio_off, _ = _split_model_output(noise_pred_audio_off)
                    noise_pred = (
                        noise_pred_nega
                        + cfg_scale_audio * (noise_pred_audio_off - noise_pred_nega)
                        + cfg_scale * (noise_pred_posi - noise_pred_audio_off)
                    )
                else:
            ...
            if depth_pred_posi is not None:
                raise NotImplementedError("scheduler_global_dwsw does not support depth-branch inference yet.")
            return noise_pred
        
        need_router_cache = (
            collect_router_logits_per_step or use_face_reference_guidance
        ) and hasattr(self.dit, 'is_moe_enabled') and self.dit.is_moe_enabled()

        # 如果需要收集 router logits，启用缓存
        if need_router_cache:
            for block in self.dit.blocks:
                if hasattr(block, 'ffn') and hasattr(block.ffn, 'enable_router_logits_cache'):
                    block.ffn.enable_router_logits_cache(True)
        
        use_scheduler_global_dwsw = (
            scheduler_global_dwsw
            and sliding_window_size is not None
            and sliding_window_stride is not None
            and inputs_shared.get("latents") is not None
            and inputs_shared["latents"].shape[2] > 2
        )
        if use_scheduler_global_dwsw:
            target_latent_len = inputs_shared["latents"].shape[2] - 1
            scheduler_window_size = min(int(sliding_window_size), int(target_latent_len))
            scheduler_stride = min(max(1, int(sliding_window_stride)), scheduler_window_size)
            scheduler_overlap = max(0, scheduler_window_size - scheduler_stride)
            print(
                "[StableAvatar DWSW] scheduler-level latent blending enabled: "
                f"target_latents={target_latent_len}, window={scheduler_window_size}, "
                f"stride={scheduler_stride}, overlap={scheduler_overlap}, "
                f"weight={scheduler_global_weight_scheme}"
            )
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if use_scheduler_global_dwsw:
                latents_all = inputs_shared["latents"]
                pred_latents = torch.zeros_like(latents_all)
                pred_latents[:, :, 0:1] = latents_all[:, :, 0:1]
                if "first_frame_latents" in inputs_shared:
                    pred_latents[:, :, 0:1] = inputs_shared["first_frame_latents"]

                start = 0
                previous_end = None
                while start < target_latent_len:
                    end = min(start + scheduler_window_size, target_latent_len)
                    window_shared = _make_scheduler_window_inputs(inputs_shared, latents_all, start, end)
                    noise_pred = _run_model_cfg(window_shared, timestep, start)
                    window_latents = self.scheduler.step(
                        noise_pred,
                        self.scheduler.timesteps[progress_id],
                        window_shared["latents"],
                    )
                    if "first_frame_latents" in inputs_shared:
                        window_latents[:, :, 0:1] = inputs_shared["first_frame_latents"]

                    if start != 0 and progress_id != 0 and previous_end is not None:
                        overlap_len = min(max(0, previous_end - start), window_latents.shape[2] - 1)
                        if overlap_len > 0:
                            weights = _scheduler_dwsw_weights(overlap_len, window_latents.device, window_latents.dtype)
                            prev_slice = pred_latents[:, :, 1 + start:1 + start + overlap_len]
                            cur_slice = window_latents[:, :, 1:1 + overlap_len]
                            window_latents[:, :, 1:1 + overlap_len] = cur_slice * weights + prev_slice * (1.0 - weights)

                    pred_latents[:, :, 1 + start:1 + end] = window_latents[:, :, 1:1 + (end - start)]
                    previous_end = end
                    if end >= target_latent_len:
                        break
                    start += scheduler_stride

                inputs_shared["latents"] = pred_latents
                continue
            ...
            step_router_logits = None
            # 如果启用了 router logits 收集，在 posi 推理后立即收集（只取 conditional branch）
            if need_router_cache:
                step_router_logits = self._collect_router_logits_from_dit(
                    self.dit,
                    cfg_merge=cfg_merge,
                    audio_only=not use_face_reference_guidance,
                )
                if step_router_logits is not None:
                    if collect_router_logits_per_step:
            ...
            if cfg_scale != 1.0 or (use_audio_native_guidance and cfg_scale_audio != 1.0):
                if cfg_merge:
                    ...
                    if use_audio_native_guidance:
                        raise NotImplementedError(
                            "cfg_merge=True is not supported together with StableAvatar-style audio native guidance. "
                            "Please set cfg_merge=False."
                        )
                    ...
                    noise_pred_nega, depth_pred_nega = _split_model_output(noise_pred_nega)

                noise_pred_audio_off = None
                depth_pred_audio_off = None
                if use_audio_native_guidance and inputs_audio_off is not None:
                    noise_pred_audio_off = self.model_fn(
                        **models,
                        **inputs_shared,
                    ...
                    noise_pred_audio_off, depth_pred_audio_off = _split_model_output(noise_pred_audio_off)

                # 主视频 CFG
                if use_audio_native_guidance and noise_pred_audio_off is not None:
                    noise_pred = (
                        noise_pred_nega
                        + cfg_scale_audio * (noise_pred_audio_off - noise_pred_nega)
                        + cfg_scale * (noise_pred_posi - noise_pred_audio_off)
                ...
                # 深度视频 CFG（如果有）
                depth_pred = None
                if use_audio_native_guidance and depth_pred_nega is not None and depth_pred_audio_off is not None and depth_pred_posi is not None:
                    depth_pred = (
                        depth_pred_nega
                        + cfg_scale_audio * (depth_pred_audio_off - depth_pred_nega)
                        + cfg_scale * (depth_pred_posi - depth_pred_audio_off)
                    )
                elif depth_pred_posi is not None and depth_pred_nega is not None:
            ...
            guidance_strength = _face_reference_guidance_strength(timestep)
            if guidance_strength > 0.0 and step_router_logits is not None:
                noid_shared = _make_noid_shared_inputs(inputs_shared)
                noise_pred_noid = self.model_fn(
                    **models,
                    **noid_shared,
                    **inputs_posi,
                    timestep=timestep,
                ...
                noise_pred_noid, _ = _split_model_output(noise_pred_noid)
                face_mask = _router_face_mask_to_latents(step_router_logits, noise_pred)
                if face_mask is not None:
                    delta_id = noise_pred_posi - noise_pred_noid
                    delta_id = _face_reference_apply_apg(delta_id, noise_pred_posi)
                    guided_delta = _face_reference_apply_samg(delta_id, face_mask)
                    noise_pred = noise_pred + guidance_strength * guided_delta
                    if not face_reference_guidance_reported:
                        print(
                            "[FaceRefGuidance] enabled: "
                            f"scale={face_reference_guidance_scale}, "
                            f"power={face_reference_guidance_power}, "
                            f"t_range=[{face_reference_guidance_start_t}, {face_reference_guidance_end_t}], "
                            f"counterfactual={face_reference_guidance_counterfactual}, "
                            f"blur_kernel={face_reference_guidance_blur_kernel}, "
                            f"samg={face_reference_guidance_samg}, "
                            f"samg_mult=[{face_reference_guidance_samg_min_mult}, {face_reference_guidance_samg_max_mult}], "
                            f"apg_eta={face_reference_guidance_apg_eta}, "
                            f"mask_mean={float(face_mask[:, :, 1:].float().mean().detach().cpu().item()):.4f}, "
                            f"delta_mean={float(delta_id.float().abs().mean().detach().cpu().item()):.6f}"
                        )
                        face_reference_guidance_reported = True

            if use_arcface_guidance and progress_id in arcface_guidance_step_ids:
                corrected_latents, arcface_changed = _arcface_guidance_update(
                    inputs_shared["latents"],
                    noise_pred,
                    progress_id,
                )
                # ArcFace guidance decodes through VAE even when it early-stops
                # without changing latents. Restore the DiT stack either way
                # before continuing denoising.
                active_iteration_models = self.in_iteration_models_2 if models.get("dit") is self.dit2 else self.in_iteration_models
                self.load_models_to_device(active_iteration_models)
                if arcface_changed:
                    inputs_shared["latents"] = corrected_latents
                    if "first_frame_latents" in inputs_shared:
                        inputs_shared["latents"][:, :, 0:1] = inputs_shared["first_frame_latents"]
                    # The old velocity is no longer consistent after changing z_t.
                    # Re-run CoInteract on the corrected latent before scheduler.step.
                    noise_pred = _run_model_cfg(inputs_shared, timestep, 0)
        ...
        if use_arcface_post_guidance:
            corrected_latents, arcface_changed = _arcface_post_guidance_update(inputs_shared["latents"])
            if arcface_changed:
                inputs_shared["latents"] = corrected_latents
                if "first_frame_latents" in inputs_shared:
                    inputs_shared["latents"][:, :, 0:1] = inputs_shared["first_frame_latents"]

        latent_output = inputs_shared["latents"].detach().clone() if return_latents else None

        # HJB-style latent refinement needs the denoised latent but performs its
        # own final decode. Skipping this decode avoids holding two decoded
        # clips in memory and keeps normal callers unchanged by default.
        if not decode_video:
            self.load_models_to_device([])
            if return_latents:
                return None, latent_output
            return None
        ...
        if depth_branch_enabled and depth_video is not None:
            if return_latents:
                return video, depth_video, latent_output
            return video, depth_video
        if return_latents:
            return video, latent_output
        return video
...
class WanVideoUnit_S2V(PipelineUnit):
    ...
    def process_motion_latents(self, pipe: WanVideoPipeline, height, width, tiled, tile_size, tile_stride, motion_video=None, motion_latents=None):
        motion_frames = 73
        kwargs = {}
        if motion_latents is not None:
            ...
            print(f"[latent-carry] using precomputed motion_latents={tuple(motion_latents.shape)}")
            return kwargs
    ...
    def process(self, pipe: WanVideoPipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        motion_latents = inputs_shared.pop("motion_latents", None)
        ...
        inputs_shared.update(self.process_motion_latents(pipe, height, width, tiled, tile_size, tile_stride, motion_video, motion_latents=motion_latents))
...
class TemporalTilerS2V_BCTHW:
    """S2V-aware temporal tiler for Dynamic Weighted Sliding-Window inference.

    S2V latents are laid out as [clean_reference, target_latents...]. A generic
    temporal crop would make later windows treat a noisy target latent as the
    reference frame, so this tiler always prepends the original clean reference
    latent and only slides over the target portion.
    """

    def build_window_mask(self, length, left_bound, right_bound, overlap, device, dtype):
        mask = torch.ones((length,), device=device, dtype=dtype)
        overlap = min(int(overlap), int(length))
        if overlap <= 0:
            return mask

        ramp = torch.linspace(0.0, 1.0, overlap, device=device, dtype=torch.float32)
        ramp = torch.log1p(ramp * (torch.exp(torch.ones((), device=device)) - 1.0))
        denom = torch.clamp(ramp.max() - ramp.min(), min=1e-6)
        ramp = ((ramp - ramp.min()) / denom).to(dtype=dtype)

        if not left_bound:
            mask[:overlap] = ramp
        if not right_bound:
            mask[-overlap:] = torch.flip(ramp, dims=(0,))
        return mask.view(1, 1, length, 1, 1)

    def slice_audio_for_window(self, audio_embeds, start, length, raw_per_latent=4):
        if audio_embeds is None:
            return None
        raw_start = int(start) * int(raw_per_latent)
        raw_len = int(length) * int(raw_per_latent) + 1
        raw_end = raw_start + raw_len
        sliced = audio_embeds[..., raw_start:raw_end]
        current_len = sliced.shape[-1]
        if current_len < raw_len:
            pad_len = raw_len - current_len
            pad_source = sliced[..., -1:] if current_len > 0 else audio_embeds[..., -1:]
            sliced = torch.cat([sliced, pad_source.repeat(*([1] * (pad_source.ndim - 1)), pad_len)], dim=-1)
        return sliced

    def run(self, model_fn, sliding_window_size, sliding_window_stride, computation_device, computation_dtype, model_kwargs):
        latents = model_kwargs["latents"]
        ref_latents = latents[:, :, 0:1]
        target_latents = latents[:, :, 1:]
        B, C, target_T, H, W = target_latents.shape
        if target_T <= 0:
            return model_fn(**model_kwargs)

        window_size = min(int(sliding_window_size), int(target_T))
        stride = max(1, int(sliding_window_stride))
        overlap = max(0, window_size - stride)
        data_device, data_dtype = latents.device, latents.dtype
        value = torch.zeros((B, C, target_T, H, W), device=data_device, dtype=data_dtype)
        weight = torch.zeros((1, 1, target_T, 1, 1), device=data_device, dtype=data_dtype)

        starts = list(range(0, target_T, stride))
        for start in starts:
            if start - stride >= 0 and start - stride + window_size >= target_T:
                continue
            end = min(start + window_size, target_T)
            target_window = target_latents[:, :, start:end].to(device=computation_device, dtype=computation_dtype)
            window_kwargs = dict(model_kwargs)
            window_kwargs["latents"] = torch.cat(
                [ref_latents.to(device=computation_device, dtype=computation_dtype), target_window],
                dim=2,
            )
            if model_kwargs.get("s2v_pose_latents") is not None:
                window_kwargs["s2v_pose_latents"] = model_kwargs["s2v_pose_latents"][:, :, start:end].to(
                    device=computation_device,
                    dtype=computation_dtype,
                )
            if model_kwargs.get("audio_embeds") is not None:
                window_kwargs["audio_embeds"] = self.slice_audio_for_window(
                    model_kwargs["audio_embeds"],
                    start=start,
                    length=end - start,
                ).to(device=computation_device, dtype=computation_dtype)
            window_kwargs["sliding_window_size"] = None
            window_kwargs["sliding_window_stride"] = None

            model_output = model_fn(**window_kwargs).to(device=data_device, dtype=data_dtype)
            model_target = model_output[:, :, 1:]
            mask = self.build_window_mask(
                model_target.shape[2],
                left_bound=(start == 0),
                right_bound=(end == target_T),
                overlap=overlap,
                device=data_device,
                dtype=data_dtype,
            )
            value[:, :, start:end] += model_target * mask
            weight[:, :, start:end] += mask

        value = value / torch.clamp(weight, min=1e-6)
        return torch.cat([latents[:, :, 0:1], value], dim=2)
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        if audio_embeds is not None:
            ...
            return TemporalTilerS2V_BCTHW().run(
                model_fn_wan_video,
                sliding_window_size,
...
def model_fn_wans2v(
    ...
    **kwargs,
    ...
    def _reference_kv_config_for_block(block_id, device):
        if hasattr(dit, "_build_reference_kv_config"):
            return dit._build_reference_kv_config(
                block_id, normalized_timestep, seq_len_x_global, f, h, w, rf, rh, rw, device
            )
        return None
    ...
    def create_custom_forward(module, seq_len_depth=0, seq_len_target=0, depth_freqs=None, depth_mutual_visible=True,
                               ...
                               depth_context=None, block_id=None):
        """
        ...
        def custom_forward(x, ctx, tm, seq_len, freq, _depth_freqs, _depth_ctx):
            ...
            return module(
                ...
                reference_kv_config=_reference_kv_config_for_block(block_id, x.device),
            )
            ...
            for blk, bid in zip(blocks, block_ids):
                ...
                def blk_forward(_x, _ctx, _tm, _freq, _df, _dc, _blk=blk, _seq=seq_len, 
                    ...
                    return _blk(
                        ...
                        depth_context=_dc,
                        reference_kv_config=_reference_kv_config_for_block(_bid, _x.device),
                    )
            ...
            for blk, bid in zip(current_blocks, current_block_ids):
                if return_router_logits:
                    ...
                    x_out = blk(
                        ...
                        seq_len_extra_ref=seq_len_extra_ref,
                        depth_context=depth_context_final,
                        reference_kv_config=_reference_kv_config_for_block(bid, x.device),
                    )
