...
OUT_DIR="$ROOT/output_videos/tia2v_case007_stableavatar_20260705"
LOG_PATH="$ROOT/logs/tia2v_case007_stableavatar_20260705.log"
...
{
  echo "===== CoInteract TIA2V StableAvatar-like case007 start $(date '+%F %T') ====="
...
env ASCEND_RT_VISIBLE_DEVICES="$VISIBLE_NPU" \
  "$PYTHON_BIN" "$ROOT/batch_infer_qilin_og_tia2v.py" \
    ...
    --cfg_scale_audio 4.0 \
    --sigma_shift 7.0 \
    --enable_stableavatar_audio_guidance \
...
echo "===== CoInteract TIA2V StableAvatar-like case007 end $(date '+%F %T') =====" | tee -a "$LOG_PATH"
