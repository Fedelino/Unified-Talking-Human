...
CSV_PATH="/data1/workspace/linxinliang/CoInteract/examples/m2v_og/cointeract_qilin_og_m2v_halfbody.csv"
OUTPUT_DIR="/data1/workspace/linxinliang/CoInteract/output_videos/qilin_og_m2v_halfbody_720p_20260622"
LOG_PATH="/data1/workspace/linxinliang/CoInteract/logs/qilin_og_m2v_halfbody_720p_20260622_resume.log"
TMP_CSV="/tmp/cointeract_qilin_og_m2v_halfbody_missing.csv"
...
while true; do
    if pgrep -f "$RUN_SIG" >/dev/null; then
        echo "[$(date '+%F %T')] halfbody OG run still active; waiting..." | tee -a "$LOG_PATH"
        sleep 120
        continue
    fi

    if all_done >>"$LOG_PATH" 2>&1; then
        echo "[$(date '+%F %T')] all halfbody OG outputs are present." | tee -a "$LOG_PATH"
        exit 0
    fi

    echo "[$(date '+%F %T')] resuming missing halfbody OG samples..." | tee -a "$LOG_PATH"
