...
VARIANTS = [
    {
        "name": "latblur_k5_samgstrong_scale1",
        "extra": [
    ...
    {
        "name": "latblur_k5_apg025_scale1",
        "extra": ["--face_reference_guidance_apg_eta", "0.25"],
    },
    {
        "name": "latblur_k5_apg0_scale1",
        "extra": ["--face_reference_guidance_apg_eta", "0.0"],
    },
]
...
def launch(variant, npu: str):
    out_dir = ROOT / "output_videos" / f"p2v_idguide_ablate_{variant['name']}_motion2_{DATE}"
    out_dir.mkdir(parents=True, exist_ok=True)
    log_path = ROOT / "logs" / f"p2v_idguide_ablate_{variant['name']}_motion2_{DATE}.log"
    env = os.environ.copy()
