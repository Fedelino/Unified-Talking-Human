...
def parse_args():
    ...
    parser.add_argument("--enable_latent_carry", action="store_true", default=False,
                        help="Carry previous clip motion memory directly as VAE latents instead of RGB frames.")
    parser.add_argument("--latent_carry_slices", type=int, default=19,
                        help="Number of final latent slices to reuse as next-clip S2V motion memory.")
    parser.add_argument("--latent_window_size", type=int, default=12,
                        help="Latent target-frame window size for DWSW. For 77-frame clips target latent length is usually 19.")
    parser.add_argument("--latent_overlap", type=int, default=4,
...
def speech_to_video(pipe, prompts, person_image, audio_path,
                    ...
                    enable_latent_carry=False,
                    latent_carry_slices=19,
    ...
    try:
        for r in range(num_repeat):
            ...
            pipe_result = pipe(
                ...
                motion_video=[] if enable_latent_carry else motion_videos,
                motion_latents=motion_latents_memory if enable_latent_carry else None,
                ...
                return_latents=enable_latent_carry,
            )
            if enable_latent_carry:
                current_clip, current_latents = pipe_result
                carry_len = min(int(latent_carry_slices), max(1, current_latents.shape[2] - 1))
                motion_latents_memory = current_latents[:, :, -carry_len:].detach().clone()
                print(f"  Latent carry clip={r+1}/{num_repeat} carry_slices={carry_len} shape={tuple(motion_latents_memory.shape)}")
            else:
                current_clip = pipe_result
    ...
    for idx, row in df.iterrows():
        try:
            ...
            speech_to_video(
                ...
                enable_latent_carry=args.enable_latent_carry,
                latent_carry_slices=args.latent_carry_slices,
