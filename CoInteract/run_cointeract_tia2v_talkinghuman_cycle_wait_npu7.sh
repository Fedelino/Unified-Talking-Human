...
wait_for_npu_free() {
    local npu_id="$1"
    while true; do
        if npu-smi info | grep -q "No running processes found in NPU ${npu_id}"; then
            break
        fi
        echo "[$(date '+%F %T')] tia2v_talkinghuman: waiting for NPU ${npu_id} to become free" | tee -a "$LOG_PATH"
        sleep "$WAIT_SECONDS"
    done
}
...
if [ "${#ROW_CSVS[@]}" -eq 0 ]; then
    ...
    for row_csv in "${ROW_CSVS[@]}"; do
        ...
        while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
            wait_for_npu_free "${ASCEND_RT_VISIBLE_DEVICES}"
            echo "[$(date '+%F %T')] tia2v_talkinghuman: running ${sample_name}, attempt ${attempt}" | tee -a "$LOG_PATH"
            if python batch_infer_qilin_og_tia2v.py \
